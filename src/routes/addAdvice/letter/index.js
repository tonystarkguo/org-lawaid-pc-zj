...
import { Button, Row, Col, Form, Input, Select, Radio, Progress, Modal, DatePicker, Card, Checkbox, message, Cascader, Upload, Icon, TreeSelect } from 'antd'
...
const Letter = ({
  ...
  const { bol, caseReason, fileModal, allConfig, formData, curTotalFileList=[] } = letter
  const { dictData } = allConfig
  const consultHistory = letter.consultHistory || []
  ...
  if(consult_source){
    ...
    consult_source[1].children = telephone
    consult_source[2].children = letter
    consult_source[3].children = visit*/

    if(consult_source.length){
      consult_source[0].children = letter
    }
  }
  ...
  const uploadProps = {
    ...
    beforeUpload: (file, fileList) => {
      dispatch({
        type: 'letter/updateProgress',
        payload: fileList,
      })
    },
    onChange({file = {}, fileList = []}) {
      dispatch({
        type: 'letter/updateFileList',
        payload: {file, fileList},
      })
    },
    onRemove(file) {
      dispatch({
          type: 'letter/removeFile',
          payload: file
      })
    },
          ...
          <Col span={12}>
            <FormItem {...formItemLayout} label="咨询类型来源">
              {getFieldDecorator('dicSource', {
                ...
                <Cascader size="large" options={consult_source} placeholder="请选择来源渠道" />
              )}
            </FormItem>
          </Col>
        ...
        <Row>
          <Col span={12}>
            <FormItem {...formItemLayout} label="咨询标题">
              {getFieldDecorator('consultTitle')(<Input/>)}
            </FormItem>
          </Col>
...
export default connect(({ letter }) => ({ letter }))(Form.create()(Letter)) 
