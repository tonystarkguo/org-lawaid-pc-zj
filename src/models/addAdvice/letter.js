...
export default {
  ...
  state: {
    ...
    formData:{
      ...
      cardCode: {value: ''},
      arrivals: {value: ''},
      dicNationality: {value: ''},
      regis: {value: ''},
      dicNation: {value: ''},
      legalInstAddr: {value: ''},
      dicEduLevel: {value: ''},
      consultantCategoryKeys: {value: []},
      consultTitle: {value: ''},
      caseReasons: {value: []},
      consultContent: {value: ''},
      dicTreatmentMode: {value: ''},
      answerSuggestion: {value: ''},
  ...
  subscriptions: {
    setup ({ dispatch, history }) {
      history.listen(() => {
        const match = pathToRegexp('/addAdvice/letter').exec(location.pathname)
        if (match) {
  ...
  effects: {
    ...
    * updateFileList ({payload}, {select, call, put}){
      let fileModal = yield select(({letter}) => letter.fileModal)
    ...
    * save({ payload }, { select, put, call }) {
      const files = yield select(({ letter }) => letter.fileModal.fileList)
    ...
    * submit({ payload }, { select, put, call }) {
      let files = yield select(({ letter }) => letter.fileModal.fileList)
  ...
  reducers: {
    ...
    showConsultNumber(state){
      return {...state, bol: true}
    },
    hideConsultNumber(state){
      return {...state, bol: false}
    },
