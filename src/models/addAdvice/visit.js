...
export default {
  ...
  subscriptions: {
    setup ({ dispatch, history }) {
      history.listen(() => {
        const match = pathToRegexp('/addAdvice/visit').exec(location.pathname)
        if (match) {
  ...
  effects: {
    ...
    * updateFileList ({payload}, {select, call, put}){
      let fileModal = yield select(({visit}) => visit.fileModal)
    ...
    /** save({ payload }, { select, put, call }) {
      const files = yield select(({ visit }) => visit.fileModal.fileList)
    ...
    * submit({ payload }, { select, put, call }) {
      let files = yield select(({ visit }) => visit.fileModal.fileList)
