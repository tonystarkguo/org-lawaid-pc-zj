...
export default {
  ...
  subscriptions: {
    setup ({ dispatch, history }) {
      history.listen(location => {
        if (location.pathname === '/aidPersonManagement') {
  ...
  effects: {
    ...
    * query ({ payload = {} }, { select, call, put }) {
      const searchKeys = yield select(({ aidPersonManagement }) => aidPersonManagement.searchKeys)
      ...
      reqParams.serviceId = 'srvid_aidPersonManagementList'

      const res = yield call(getDataService, {url: api.aidPersonManagementList}, {...reqParams, ...criterias})
    ...
    * import ({ payload }, { call, put }) {
      const res = yield call(getDataService, {url:api.layerList}, { ...payload ,serviceId: 'srvid_layerList'})
      if (res.success) {
        let resData = res && res.data && res.data.list || []
        let pageNum = pageNum || 1
        let pageSize = 5
    ...
    * create ({ payload }, { call, put }) {
      const res = yield call(postDataService, {url:api.addLayer}, [payload])
      if (res.success) {
    ...
    * add ({ payload }, { select, call, put }) {
      const res = yield call(postDataService, {url:api.addNewLayer}, {...payload, serviceId: 'srvid_addNewLayer'})
      if(res.code === '40015'){
    ...
    * edit ({ payload }, { select, call, put }) {
      const id = yield select(({ aidPersonManagement }) => aidPersonManagement.currentItem.id)
      console.log(id)
      const params = { ...payload, id }
      const res = yield call(postDataService, {url:api.updateNewLayer}, {...params, serviceId: 'srvid_updateNewLayer'})
      if (res.success) {
        yield put({ type: 'hideAddModal' })
        yield put({ type: 'query' })
      } else {
    ...
    * back ({ payload }, { select, call, put }) {
      ...
      const res = yield call(postDataService, {url:api.operationLayer}, {...params, serviceId: 'srvid_operationLayer'})
      if (res.success) {
        yield put({ type: 'query' })
      } else {
        throw res
    ...
    * updateFileList ({payload}, {select, call, put}){
      let fileData = yield select(({ aidPersonManagement }) => aidPersonManagement.fileData)
